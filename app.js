...
function switchView(viewName) {
    ...
    if (pageTitle) {
        pageTitle.textContent = titles[viewName] || 'System Workspace';
    }
}
...
function renderDashboard() {
    ...
    const statPipe = document.getElementById('statPipe');
    const statFitting = document.getElementById('statFitting');
    const statTrials = document.getElementById('statTrials');
    if (statPipe) statPipe.textContent = pipeCount;
    if (statFitting) statFitting.textContent = fittingCount;
    ...
    const pipeBar = document.getElementById('pipeBar');
    const pipePercent = document.getElementById('pipePercent');
    if (pipeBar) pipeBar.style.width = `${pipePct}%`;
    if (pipePercent) pipePercent.textContent = `${pipePct}%`;

    const fittingBar = document.getElementById('fittingBar');
    const fittingPercent = document.getElementById('fittingPercent');
    if (fittingBar) fittingBar.style.width = `${fittingPct}%`;
    if (fittingPercent) fittingPercent.textContent = `${fittingPct}%`;

    const commonBar = document.getElementById('commonBar');
    const commonPercent = document.getElementById('commonPercent');
    if (commonBar) commonBar.style.width = `${commonPct}%`;
    if (commonPercent) commonPercent.textContent = `${commonPct}%`;
...
function setupRecordsView() {
    ...
    if (searchInput) searchInput.addEventListener('input', renderRecordsList);
    if (filterDept) filterDept.addEventListener('change', renderRecordsList);
    if (filterType) filterType.addEventListener('change', renderRecordsList);
...
window.viewRecordDetails = function(id) {
    ...
    if (dialogTitle) dialogTitle.textContent = `${record.product || 'Record'} (${record.machine || 'General'})`;
...
function setupMaterialsView() {
    const printMaterialsBtn = document.getElementById('printMaterialsBtn');
    if (printMaterialsBtn) {
        printMaterialsBtn.addEventListener('click', () => {
            window.print();
        });
    }
}
...
function setupPdfExportHandlers() {
    ...
    if (btnCancelExport && pdfFilterModal) {
    ...
    if (btnConfirmExport) {
        btnConfirmExport.addEventListener('click', () => {
            ...
            if (pdfFilterModal) pdfFilterModal.style.display = 'none';
        });
    }
